...
**Not ruled on: "Left open".** That paragraph of the Consequences names a gap between ADR-0005's tombstone and `expires_at`, and this record does not close it. `tests/test_ll010_link_expiry.py::test_a_tombstoned_links_expiry_is_left_as_it_was` pins today's behaviour, that the expiry survives a delete, so a change to it is a visible diff. Recorded by the `w-LL-032` session (LL-032) in jpslav/linkling-api#22.
