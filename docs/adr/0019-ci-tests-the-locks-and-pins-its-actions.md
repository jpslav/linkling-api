...
Accepted 2026-09-25 by `pm-3`, the program manager, under the 2026-09-22 ruling in `company/DECISIONS.md` of the program repo ("What reaches the owner is user-visible impact, not cost to undo"): what CI installs before it runs pytest, and how its actions are pinned, are not something a user of Linkling perceives. `pm-3`'s dispatch of LL-032 made accepting it conditional on its matching `ci.yml` and the lock files on `main`, and it does. Proposed since 2026-09-25. Checked against `main` at `ac4e085` before accepting: the `test` job in `.github/workflows/ci.yml` runs `pip install --require-hashes -r requirements.lock.txt -r requirements-test.lock.txt -r requirements-build.lock.txt`, then `pip install --no-deps --no-build-isolation -e .`, then `pytest -q`; `no-forbidden-imports` installs the image lock and the build lock only; and the one install that fetches an unhashed package is its `pip install --no-cache-dir websockets` (the two `pip install --no-deps --no-build-isolation` lines carry no `--require-hashes`, and the editable install completes with `--no-index`, so it fetches nothing). Every `uses:` is a 40-hex commit with a `# vX.Y.Z` comment, and `git ls-remote --tags` puts `actions/checkout`'s `v4` and `v4.4.0` at `11d5960a` and `actions/setup-python`'s `v5` and `v5.6.0` at `a26af69b`, the two commits `ci.yml` names. `requirements.lock.txt` holds neither `pytest` nor `httpx`; `requirements-test.lock.txt` repeats all 13 of its pins at the same versions and hashes and adds 8 packages, `pytest` and `httpx` among them. `.github/dependabot.yml` has the `github-actions` block, weekly on Mondays, one group `actions`, and the `gh api` read in the last Consequence, run again, still requires only `test`. `tests/test_ll030_locked_ci.py` and `tests/test_ll024_pins_refresh.py` pass (16 passed) in a Python 3.12 virtualenv built from the three locks under `--require-hashes`.

One thing ii says was not made has since been made. Dependabot's run on the `github-actions` block is api #21 (opened by Dependabot on 2026-09-25, open at the time of writing): it moves `actions/checkout` to `v7.0.1` and `actions/setup-python` to `v7.0.0` and rewrote each trailing comment with its SHA (`# v4.4.0` to `# v7.0.1`, `# v5.6.0` to `# v7.0.0`), which is what ii read from dependabot-core. A move of a major is a PR to read, as ii says, and this record does not judge that one. Recorded by the `w-LL-032` session (LL-032) in jpslav/linkling-api#22.
