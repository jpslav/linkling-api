...
- A clean checkout of `linkling-api` means two clones side by side. CI can build only the
  ...
  there.)*
  > ⚠️ **Superseded 2026-09-25 — LL-034.** `linkling-web` was private when this record was
  > written (its Context, and the reason given for alternative B). It became public on
  > 2026-09-25, so a workflow reads it with no deploy key or secret, and CI now builds the real
  > site too: the `no-third-party-site` job in `.github/workflows/ci.yml` checks out
  > `jpslav/linkling-web` at `main` and runs the whole no-third-party check against it, beside
  > the stand-in, which stays for the fail and blind paths a clean site cannot show. "CI can
  > build only the service" and
